...
std::string usersToJson(const std::vector<User>& users) {
    ...
    oss << "]," << "\"total\":" << users.size() << ","
        << "\"source\":\"users_service C++ executable\"," << "\"endpoint\":\"/users\","
...
int main(int argc, char* argv[]) {
    ...
    std::vector<User> users = {{1, "João Silva", "joao@example.com"},
    ...
    if (method == "GET") {
        if (path == "/users" || path == "/") {
            ...
            try {
                ...
                // User not found
                std::cout << "{" << "\"error\":\"User not found\"," << "\"userId\":" << userId
                          << "," << "\"source\":\"users_service C++ executable\"" << "}"
                          << std::endl;
                return 1;
        ...
        std::cout << "{" << "\"message\":\"User created successfully\","
    ...
    std::cout << "{" << "\"error\":\"Method not allowed\"," << "\"method\":\"" << method << "\","
              << "\"path\":\"" << path << "\"," << "\"source\":\"users_service C++ executable\""
              << "}" << std::endl;
    return 1;
}
