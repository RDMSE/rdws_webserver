...
std::string orderToJson(const Order& order) {
    ...
    oss << "{" << "\"id\":" << order.id << "," << "\"userId\":" << order.userId << ","
...
int main(int argc, char* argv[]) {
    ...
    std::vector<Order> orders = {{1, 1, "Laptop Dell", 2500.00, "completed"},
        ...
        if (path == "/orders" || path == "/") {
            ...
            try {
                ...
                for (const auto& order : orders) {
                    if (order.id == orderId) {
                        std::cout << "{" << "\"order\":" << orderToJson(order) << ","
                ...
                std::cout << "{" << "\"error\":\"Invalid order ID\"," << "\"path\":\"" << path
                          << "\"," << "\"source\":\"orders_service C++ executable\"" << "}"
                          << std::endl;
                return 1;
            }
            ...
            try {
                ...
                oss << "]," << "\"total\":" << userOrders.size() << ","
                    << "\"source\":\"orders_service C++ executable\"," << "\"endpoint\":\"" << path
                    << "\"," << "\"timestamp\":" << std::time(nullptr) << "}";
                ...
                std::cout << "{" << "\"error\":\"Invalid user ID or path format\"," << "\"path\":\""
                          << path << "\"," << "\"source\":\"orders_service C++ executable\"" << "}"
                          << std::endl;
                return 1;
            }
        }
    ...
    std::cout << "{" << "\"error\":\"Method not allowed\"," << "\"method\":\"" << method << "\","
              << "\"path\":\"" << path << "\"," << "\"source\":\"orders_service C++ executable\""
              << "}" << std::endl;
    return 1;
}
