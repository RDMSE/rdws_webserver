...
std::string ResponseHelper::returnError(const std::string& message, int statusCode, const ::rapidjson::Value* details) {
    ::rapidjson::Document doc;
    doc.SetObject();
    auto& allocator = doc.GetAllocator();
...
std::string ResponseHelper::returnData(const ::rapidjson::Value& data, const std::string& message, int statusCode) {
    ::rapidjson::Document doc;
    doc.SetObject();
    auto& allocator = doc.GetAllocator();
...
void ResponseHelper::addMetadata(::rapidjson::Document& doc, ::rapidjson::Document::AllocatorType& allocator, const std::string& source) {
    ...
    doc.AddMember("timestamp", ::rapidjson::Value(static_cast<int64_t>(std::time(nullptr))), allocator);
}
